...
impl<T: RenderTarget> Canvas<T> {
    ...
    pub fn draw_debug_text<P: Into<FPoint>>(&mut self, string: &str, point: P) -> Result<(), Error> {
        let string =  CString::new(string).unwrap();
        let point = point.into();
        let result = unsafe { sys::render::SDL_RenderDebugText(self.context.raw, point.x, point.y, string.as_ptr() as *const c_char) };
        if !result {
            Err(get_error())
        } else {
            Ok(())
