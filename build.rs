...
fn main() {
    ...
    // Workaround: sdl3-net-sys 0.6.0 ships a pkg-config file with an empty
    // `prefix=`, so the link search path that sdl3-net-sys emits resolves to
    // `/lib`. Add the build directory of sdl3-net-sys back in so rustc can
    // actually find libSDL3_net.{so,a}.
